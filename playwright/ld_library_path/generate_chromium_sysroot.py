"""Generate a .bzl file with Chromium sysroot packages and a module extension.
...
_BZL_TEMPLATE = '''\
"""Chromium sysroot packages for Playwright LD_LIBRARY_PATH support.
...
# AUTO-GENERATED by:
#
#   bazel run @rules_jupyter//playwright/ld_library_path:generate_chromium_sysroot -- \\
#       --output <path> --release {release}
#
# DO NOT MODIFY BY HAND.

load("@rules_jupyter//playwright/ld_library_path:sysroot.bzl", "playwright_ld_library_dir_repo")
load("@rules_jupyter//tools/debian:debian_archive.bzl", "debian_archive")

CHROMIUM_SYSROOT_PACKAGES = {packages}

def _chromium_sysroot_impl(module_ctx):
    """Create debian_archive repos and ld_library_dir repos for each platform."""

    # buildifier: disable=unused-variable
    _unused = module_ctx

    for platform, packages in CHROMIUM_SYSROOT_PACKAGES.items():
        suffix = platform.replace("-", "_")
        deb_labels = []
        for pkg in packages:
            repo_name = "chromium_sysroot_%s_%s" % (
        ...
        playwright_ld_library_dir_repo(
            name = "chromium_sysroot_ld_library_dir_%s" % suffix,
            deps = deb_labels,
        )

chromium_sysroot = module_extension(
...
def main() -> None:
    """Generate a .bzl file with sysroot packages and module extension."""
    logging.basicConfig(
    ...
    if module_bazel:
        ...
        repo_names = [
            f"chromium_sysroot_ld_library_dir_{p.replace('-', '_')}"
            for p in sorted(packages_by_platform.keys())
        ]
        repos_str = ",\n    ".join(f'"{r}"' for r in repo_names)
        print()
        print("# Add to MODULE.bazel:")
        print(f'chromium_sysroot = use_extension("{label}",' ' "chromium_sysroot")')
        print("use_repo(")
        print("    chromium_sysroot,")
        print(f"    {repos_str},")
        print(")")
        print()
    else:
